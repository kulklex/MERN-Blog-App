...
export default function SinglePostComponent({post}: Props) {
  ...
  const handleUpdate = async () => {
    try {
      await axios.put(`/posts/${postFetched?._id}`,  {email: user?.user.email, title, desc})
  ...
  const handleDelete = async () => {
   try {
    await axios.delete(`/posts/${postFetched?._id}`, {data: {email: user?.user.email}})
    navigate("/")
   } catch (error) {
    console.log(error)
   }
  ...
  return (
    <div className="singlePostComponent">
      <div className="singlePostCompWrapper flex flex-col p-5 pr-0">
        ...
        /> {
          updateMode ? <input type="text" value={title} className=" text-xl m-2 font-serif text-center focus:outline-none" autoFocus
          ...
          {postFetched.title}
            ...
            {postFetched.email === user?.user.email && (<div className="singlePostCompEdit flex justify-end mr-10">
            <svg
        ...
        </div>
        {
          ...
          <textarea className="text-sm  first-letter:text-lg first-letter:capitalize mr-10  border-none focus:outline-none  p-5 w-[80%] h-[100vh] md:my-8"  value={desc} onChange={(event: React.FormEvent<HTMLTextAreaElement>) => {
          event.preventDefault()
          setDesc(event.currentTarget.value)}}/>
          <button className="singlePostCompButton flex flex-col my-14 bg-blue-500 p-1 text-white border rounded-md justify-center object-center items-center self-center text-center" onClick={handleUpdate}>Update</button>
          </div>) : 
          <div>
